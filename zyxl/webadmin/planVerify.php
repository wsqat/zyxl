...
?>
        ...
        <div class="mainer">
                    ...
                    <li class="active"><a href="content.php" class="icon-file-text"> 内容</a>
                        <ul>
                            ...
                            if($_SESSION["u_sh"]==1){
                                ...
                                echo "<li class='active'><a href='planVerify.php'>计划审核</a></li>";
                            }
                            if($_SESSION["user_role"]==10){
                                echo "<li><a href='content.php'>内容发布</a></li>";
            ...
            <div class="admin-bread">
                ...
                <ul class="bread">
                    ...
                    <li><a href="planVerify.php">计划审核</a></li>
                </ul>
            </div>
        </div>
    </div>
    ...
    <div class="admin">
        <form method="post" action="../admin/batchdel.php">
            <div class="panel admin-panel">
                <div class="panel-head"><strong>计划审核</strong></div>
                ...
                <table class="table table-hover">
                    <tr><th width="100">选择</th><th width="120">状态</th><th width="*">院校编号</th><th width="*">院校名称</th><th width="200">操作</th></tr>
         ...
         -->
                    ...
                    for($index=0;$index<count($article_yxdm);$index++){
                        if($s==10){
                            $sql ="select yxdm,pstate from ".$yx_table." where yxdm='".$article_yxdm[$index]."' and pstate=0 ";
                        }elseif($s==11){
                            $sql ="select yxdm,pstate from ".$yx_table." where yxdm='".$article_yxdm[$index]."' and pstate=1 ";
                        }elseif($s==9){
                            $sql ="select yxdm,pstate from ".$yx_table." where yxdm='".$article_yxdm[$index]."' and pstate=-1 ";
                        }else{
                            $sql ="select yxdm,pstate from ".$yx_table." where yxdm='".$article_yxdm[$index]."'";
                        }


                        $result=@mysql_query($sql) or die("error  出错了，请联系管理员1");
                        if($result&&mysql_num_rows($result)>0){
                            $pageSize = 10;
                            $curpage = 1;
                            $countPage = 0;
...
//                                $id = $row["bh"];
                                ...
                                $array = mysql_fetch_assoc(mysql_query("select mc,bh from yx_xx where dm = '".$dm."' "));
                                $mc = $array["mc"];
                                $id = $array["bh"];

                                $state=$row['pstate'];
                                $aid = base64_encode($dm);
                                //操作加密
                                ...
                                if ($i > ($curpage - 1) * $pageSize && $i <= $curpage * $pageSize) {
                                    if($state==1){
                                        echo "<tr><td><input type='checkbox'  name='chk[]' value=".$id." /></td><td><a class='button border-blue button-little' > 已审核</a></td>
                                        <td><a href='../admin/addplan.php?aid=".$aid."'>".$dm."</a></td>
                                        <td>".$mc."</td><td><a class='button border-blue button-little' href='../admin/addplan.php?aid=".$aid."'>查看</a>
                                        <a class='button border-yellow button-little' href='../admin/delePlanCom.php?aid=".$aid."' onclick='{if(confirm('确认删除?')){return true;}return false;}'>删除</a></td></tr>";
                                    }if($state==0){
                                        echo "<tr><td><input type='checkbox' name='chk[]' value=".$id." /></td><td><a class='button border-blue button-little' > 审核中 </a></td>
                                        <td><a href='../admin/addplan.php?aid=".$aid."'>".$dm."</a></td><td>".$mc."</td>
                                        <td><a class='button border-blue button-little' href='../admin/addplan.php?aid=".$aid."'>审核</a>
                                        <a class='button border-yellow button-little' href='../admin/delePlanCom.php?aid=".$aid."' onclick='{if(confirm('确认删除?')){return true;}return false;}'>删除</a></td></tr>";
                                    }if($state==-1){
                                        echo "<tr><td><input type='checkbox' name='chk[]' value=".$id." /></td><td><a class='button border-blue button-little' > 未通过 </a></td>
                                        <td><a href='../admin/addplan.php?aid=".$aid."'>".$dm."</a></td><td>".$mc."</td>
                                        <td><a class='button border-blue button-little' href='../admin/addplan.php?s=".$up."&aid=".$aid."'>审核</a>
                                        <a class='button border-yellow button-little' href='../admin/delePlanCom.php?aid=".$aid."' onclick='{if(confirm('确认删除?')){return true;}return false;}'>删除</a></td></tr>";
                                    }
                                }
                            }
