            ...
            <ul class="nav nav-inline admin-nav">
                ...
                <li class="active"><a href="content.php" class="icon-file-text"> 内容</a>
                    ...
                    <?php
                        ...
                        if($_SESSION["u_sh"]==1){
                            echo "<li class='active'><a href='contentadmin.php'>简章审核</a></li>";
...
<div class="admin">
    ...
    <div class="panel admin-panel">
        ...
        <table class="table table-hover">
            <tr><th width="100">选择</th><th width="120">状态</th><th width="120">分类</th><th width="*">文章名称</th><th width="*">院校名称</th><th width="200">时间</th><th width="200">操作</th></tr>
...
//print_r($article_yxdm);
    ...
    for($index=0;$index<count($article_id);$index++){ 
		if($s==10){
	        $sql = "select * from ".$yx_table." where article_author_id= ".$article_id[$index]." and (class_id=3 or class_id=4) and state=0 order by article_id desc  "; 
		}elseif($s==11){
            $sql = "select * from ".$yx_table." where article_author_id= ".$article_id[$index]." and (class_id=3 or class_id=4) and state=1 order by article_id desc  "; 
		}elseif($s==9){
            $sql = "select * from ".$yx_table." where article_author_id= ".$article_id[$index]." and (class_id=3 or class_id=4) and state=-1 order by article_id desc  ";
        }else{
            ...
            while($row = mysql_fetch_array($result)) {
                ...
                if ($i > ($curpage - 1) * $pageSize && $i <= $curpage * $pageSize) {
                    ...
                    // echo "<tr><td>".$row["article_id"]."</td><td><a class='button border-blue button-little' > 审核中 </a></td><td>".$cid."</td><td><a href='../admin/look.php?aid=".$row['article_id']."'>".$row['article_title']."</a></td><td>".$row['article_time']."</td><td><a class='button border-blue button-little' href='../admin/update.php?aid=".$row['article_id']."'>修改</a> <a class='button border-yellow button-little' href='../admin/dele.php?aid=".$row['article_id']."' onclick='{if(confirm('确认删除?')){return true;}return false;}'>删除</a></td></tr>";
                        ...
                        echo "<tr><td><input type='checkbox' name='chk[]' value=".$row['article_id']." /></td><td><a class='button border-blue button-little' > 未通过 </a></td><td>".$cid."</td><td><a href='../admin/look.php?s=".$lo."&aid=".$aid."'>".$row['article_title']."</a></td><td>".$mc."</td><td>".$row['article_time']."</td><td><a class='button border-blue button-little' href='../admin/shenhe.php?s=".$up."&aid=".$aid."'>审核</a> <a class='button border-yellow button-little' href='../admin/opupdate.php?s=".$fb."&aid=".$aid."'>通过</a> <a class='button border-yellow button-little' href='../admin/opupdate.php?s=".$fb."&aid=".$aid."'>不通过</a></td></tr>";
                        // echo "<tr><td>".$row["article_id"]."</td><td><a class='button border-blue button-little' > 审核中 </a></td><td>".$cid."</td><td><a href='../admin/look.php?aid=".$row['article_id']."'>".$row['article_title']."</a></td><td>".$row['article_time']."</td><td><a class='button border-blue button-little' href='../admin/update.php?aid=".$row['article_id']."'>修改</a> <a class='button border-yellow button-little' href='../admin/dele.php?aid=".$row['article_id']."' onclick='{if(confirm('确认删除?')){return true;}return false;}'>删除</a></td></tr>";
                    }
                }
            }
