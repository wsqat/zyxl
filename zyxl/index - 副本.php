    ...
    <div class="row">
        ...
        <div class="col-lg-8">
            ...
            <ul id="myTab" class="nav nav-tabs" role="tablist">
                ...
                <li class="<?php if($_GET['schoolname']) echo  "active"; ?>"><a href="#profile" role="tab" data-toggle="tab">学校信息</a></li>
                <li class="<?php if($_GET['placeid']) echo  "active"; ?>"><a href="#profile1" role="tab" data-toggle="tab">按地区查询</a></li>
...
<table class="table table-hover table-responsive">
    ...
    if(!$_GET['placename'])
        ...
        $sql = "select * from ".$zy_table." where  sq='".$getplacename."';  ";
    ...
    $result = mysql_query($sql);
        ...
        $unique_data = array_unique ( $data ); 
        $place2total=count($unique_data);
        $place2pagenum=ceil($place2total/$num);  //获得总页数 pagenum
        //假如传入的页数参数apge 大于总页数 pagenum，则显示错误信息
        If($place2page>$place2pagenum || $place2page == 0){
            Echo "暂无数据.";
       //Exit;
        }
    $place2offset=($place2page-1)*$num;

    $place2prev_page=$place2page-1;                             //前一页
    $place2next_page=$place2page+1;                             //下一页

    //echo $_GET['schoolname'];
    if($_GET['placename']=='index')
        echo "<tr  class='info'><td><a href='#'>以地区检索</a></td><td>共".$place2total."个学校</td><td></td></tr>"; 
    else if($_GET['placename']=='')
        echo "<tr  class='info'><td><a href='#'>以地区检索</a></td><td>共".$place2total."个学校</td><td></td></tr>"; 
    else echo "<tr  class='info'><td><a href='#'>以&nbsp;".$getplacename."&nbsp;检索</a></td><td></td><td>共".$place2total."个学校</td></tr>"; 

            $i=0;
            $lastset=$place2offset+$num;
            //echo $place2offset;
            //echo $lastset;
            
            foreach($unique_data as $key1=>$value1){
                $i++;
                if($i>$place2offset&$i<=$lastset){
                    if(($i%2)==1)
                        echo "<tr>
                            <td><a href='http://".$guanwang."' target='_blank'>".$value1."</a></td>
                            <td><a href='zsjz.php?schoolid=".$sch_id."' target='_blank'>招生简章</a></td>
                            <td><a href='zsjh.php?schoolid=".$sch_id."' target='_blank'>招生计划</a></td>
                        </tr>"; 
                    if(($i%2)==0)
