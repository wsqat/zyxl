...
<div class="container">
    <div class="jumbotron">
        <img src="images/ahzcj.bmp" alt="安徽" id="bg">
        <h1>
         </h1>
    ...
    <div class="row">
        ...
        $sql = "select * from ".$zy_table." where zymc='".$_GET['zy']."'  ";
        $result = mysql_query($sql);
        $data = array();
         while($row=mysql_fetch_array($result))
        {
            echo "<tr><td>".$row["xxmc"]."</td><td></td><td>".$row["xxxz"]."</td><td>".$row["xxjb"]."</td></tr>"; 
        }
...
<?php
        ...
        foreach($unique_data as $key=>$value){
            ...
            foreach($unique_data1 as $key1=>$value1){
                ...
                if(($i%3)==0)
                    echo "<td><a href='index2.php?zy=".$value1."'>".$value1."</a></td></tr>";
                $i++;
