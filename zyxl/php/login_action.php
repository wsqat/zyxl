...
//	header("Content-type: text/html; charset=gbk2312");
	...
	$user_role='user_role';
	$username=addslashes(isset($_POST['user_name']) ? $_POST['user_name'] : '');
	$password=addslashes(isset($_POST['user_password']) ? $_POST['user_password'] : '');
	$checkcode=addslashes(isset($_POST['checkcode']) ? $_POST['checkcode'] : '');
	if(empty($username)||empty($password)){
			echo_message("用户名和密码不能为空！",-1);
	}else{
		if($checkcode!=$_SESSION['check_pic']){
				echo_message("验证码错误！",-1);
				return 0;
		}
	}

	$query="select * from ".$user_role." where user_name='$username' AND user_pw=md5('$password')";
